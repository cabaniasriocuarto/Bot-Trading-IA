...
- Bloque 9 parcial ampliado:
  ...
  - `GET /api/v1/bots/{bot_id}/experience` ya resume episodios atribuidos, exclusiones, legacy/stale, fuentes y top estrategias por bot
  - frontend `Bots` ya muestra una tarjeta visible `Experiencia atribuida al bot`
...
- Cerrar bloque 9 parcial con:
  - experiencia atribuida al bot visible
  - docs/truth consistentes
  - validacion completa del contrato nuevo
...
- la UI aun no tiene drilldown por episodio individual con `run_id`, `dataset_hash` y detalle completo de attribution
