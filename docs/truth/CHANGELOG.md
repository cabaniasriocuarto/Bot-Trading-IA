...
- Diagnostico real cerrado en Railway staging:
  ...
  - el problema no era path ni permisos;
  - el filesystem estaba saturado (`421M/434M`, `100%`) por crecimiento de `/app/user_data/instruments/registry.sqlite3` (~`419M`).
- Correccion operativa minima aplicada:
  - se elimino solo `registry.sqlite3` por ser un artefacto derivado del instrument registry;
  - se reinicio el servicio staging;
  - no hizo falta resize de volumen en este bloque.
- Resultado real despues del saneamiento:
  - el volumen quedo en uso razonable (`241M/434M`, `57%`);
  - `registry.sqlite3` se regenero en ~`240M`;
  - `POST /api/v1/instruments/registry/sync` volvio a `sync_ok=true` para:
    - `spot`
    - `margin`
    - `usdm_futures`
    - `coinm_futures`
  - `exchange_filters_fresh=true`;
  - en la lectura inmediata posterior ya no aparece `exchange_filters_blocker`.
