...
- Se fija `mock` como modo operativo visible del bot en UI/backend, separado de:
  ...
  - `testnet`
  - `live`
- `shadow` deja de mostrarse como modo principal al usuario y queda como:
  - fuente interna/canonica de experiencia
  - compatibilidad de endpoints `learning/shadow/*`
- `gates.yaml` incorpora `execution_modes.mock`.
- `experience_store.py` normaliza aliases:
  - `mock -> shadow`
  - `test -> testnet`
- `RegistryDB` extiende:
  - `instrument_registry.mock_enabled`
  - `instrument_catalog_snapshot_item.mock_enabled`
  - y expone `mode_capabilities` por instrumento/snapshot.
- `BinanceCatalogSyncService` y `InstrumentCatalogStore` ya persisten `mock_enabled`.
- `web/app.py` normaliza `shadow -> mock` en modos operativos de bot sin romper compatibilidad de experience source.
- Frontend actualizado:
  - `Strategies`: botones/labels/status de mock y experiencia `live`
  - `Execution`: filtros y acciones por `mock`
  - `Backtests`: historial por fuente incluye `live` y preferencia de bot `mock`
- Validacion:
  - `py_compile` backend PASS
  - `pytest` store/catalog/learning PASS
...
- `.github/workflows/security-ci.yml`:
  ...
  - `scripts/security_scan.sh` ahora toma baseline canÃƒÂ³nica en `docs/security/gitleaks-baseline.json` (o `GITLEAKS_BASELINE_PATH`), evitando depender de archivos `artifacts/` no versionados en CI.
...
- `scripts/ops_protected_checks_report.py`:
  ...
  - nuevo flag `--no-strict` para override explÃƒÂ­cito.
- `rtlab_autotrader/tests/test_web_live_ready.py`:
...
### AP-BOT-1004 (runtime testnet sin fill sintÃƒÂ©tico)
- `rtlab_autotrader/rtlab_core/web/app.py`:
...
- Bloque benchmark `/api/v1/bots` (regresiÃƒÂ³n local + endurecimiento remoto):
  - `scripts/benchmark_bots_overview.py` agrega:
    - `--ask-password`,
    - `--require-evidence` (exit `2` cuando no hay cardinalidad mÃƒÂ­nima),
...
- Bundle remoto de cierre (operaciÃƒÂ³n en 1 comando):
...
- `scripts/security_scan.sh` corregido:
  ...
  - elimina falso warning de Ã¢â‚¬Å“gitleaks no instaladoÃ¢â‚¬Â cuando el binario esta presente.
- CI de seguridad:
...
- Si `safe_update.gates_file` en knowledge difiere de la canÃƒÂ³nica, se agrega warning y se fuerza la fuente canÃƒÂ³nica.
- `gates_summary` en `/api/v1/config/learning` ahora expone:
  - `source` (ruta canÃƒÂ³nica efectiva)
  - flags `pbo_enabled` / `dsr_enabled` derivados de polÃƒÂ­tica canÃƒÂ³nica.
- `LearningService` para recomendaciones usa thresholds de gates canÃƒÂ³nicos:
  - `pbo_max` desde `reject_if_gt`
  - `dsr_min` desde `min_dsr`
  - fallback explÃƒÂ­cito a `knowledge/policies/gates.yaml` solo si no existe config.
- `MassBacktestEngine` aÃƒÂ±ade trazabilidad de polÃƒÂ­tica en artefactos:
...
- `Backtests > Research Batch` ahora muestra en leaderboard:
  - columna `MÃƒÂ­n trades/sÃƒÂ­mbolo`
  - badge de mÃƒÂ­nimo requerido cuando existe en `gates_eval.min_trade_quality`.
- Drilldown de variante agrega:
  - `MÃƒÂ­n trades/sÃƒÂ­mbolo` con umbral requerido
  - detalle `Trades OOS por sÃƒÂ­mbolo` (mapa agregado).
...
- `MassBacktestEngine` ahora resuelve surrogate por policy con trazabilidad:
  - nuevo resolver `_resolve_surrogate_adjustments(cfg)` con:
    ...
    - `evaluation_mode`
- Policy canÃƒÂ³nica agregada en `config/policies/gates.yaml`:
...
### Auditoria comitÃƒÂ© + hardening adicional
- Seguridad:
  - `current_user` ahora ignora headers internos si no existe `INTERNAL_PROXY_TOKEN` vÃƒÂ¡lido (fail-closed en todos los entornos).
  - login backend con rate-limit/lockout por `IP+user`:
    - `10` intentos en `10` min -> `429`
    - lockout `30` min tras `20` fallos.
  - BFF (`[...path]` y `events-stream`) ahora falla con `500` explÃƒÂ­cito si falta `INTERNAL_PROXY_TOKEN`.
- Config:
...
- BibliografÃƒÂ­a:
  - nuevo `scripts/biblio_extract.py` (indexaciÃƒÂ³n SHA256 + extracciÃƒÂ³n incremental a txt).
  - nuevo `docs/reference/biblio_txt/.gitignore`.
  - `docs/reference/BIBLIO_INDEX.md` regenerado con hashes.
- Seguridad documental:
  - `docs/SECURITY.md` reescrito con threat model mÃƒÂ­nimo, checklist y comandos de auditorÃƒÂ­a.
- Test agregado:
...
- `MassBacktestEngine` desactiva surrogate adjustments por defecto:
  ...
  - surrogate solo segun policy canÃƒÂ³nica (`gates.surrogate_adjustments`) y modo permitido.
...
- UI:
  ...
  - `execution/page.tsx`:
    - botÃƒÂ³n masivo `Modo LIVE` bloqueado cuando checklist LIVE no estÃƒÂ¡ en PASS
    - tabla de operadores agrega columnas `Sharpe` y `Kills (total/24h)`.
- Tipos frontend actualizados en `src/lib/types.ts` para nuevos campos de mÃƒÂ©tricas de bot.
- Tests:
...
- Se agregÃƒÂ³ `sort_by=winrate` en backend (`BacktestCatalogDB.query_runs`) para ranking directo por winrate.
- Se eliminÃƒÂ³ el lÃƒÂ­mite rÃƒÂ­gido de `5` en selecciÃƒÂ³n legacy (`selectedRuns`) para no recortar comparaciones manuales.
- Parser de errores UI reforzado (`uiErrMsg`) para evitar renderizar `[object Object]` y mostrar mensajes ÃƒÂºtiles (`detail/message/error/cause`).
...
- Frontend (`Backtests > Research Batch`):
  ...
  - restauraciÃƒÂ³n automÃƒÂ¡tica de shortlist al seleccionar un batch.
  - en tabla de Batches se muestra columna `Shortlist` (cantidad guardada) y acciÃƒÂ³n `Cargar shortlist`.
- Tests:
...
- Se removieron solo artefactos locales no versionados y engaÃƒÂ±osos:
  - `tmp/`
