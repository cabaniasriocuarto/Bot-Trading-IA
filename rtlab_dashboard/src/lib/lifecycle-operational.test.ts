...
import {
  ...
  summarizeLifecycleOperationalUniverse,
...
describe("lifecycle-operational", () => {
  ...
  it("resume el universo auditable para el tercer consumidor minimo en backtests", () => {
    const universeSummary = summarizeLifecycleOperationalUniverse(
      {
        contract_version: "rtlops81/v1",
        bot_id: "BOT-001",
        domain_type: "spot",
        ...
        policy_state: {
          ...
          status: "active",
          engine: "fixed_rules",
          universe: ["ethusdt", "btcusdt", "solusdt"],
          pool_strategy_ids: ["strat-a"],
          notes: "",
          created_at: "2026-04-21T12:00:00Z",
          updated_at: "2026-04-21T12:00:00Z",
    ...
    expect(universeSummary).not.toBeNull();
    expect(universeSummary?.allowedCount).toBe(1);
    expect(universeSummary?.rejectedCount).toBe(1);
    expect(universeSummary?.progressingCount).toBe(1);
    expect(universeSummary?.pausedCount).toBe(1);
    expect(universeSummary?.untrackedCount).toBe(1);
    expect(universeSummary?.symbols).toEqual([
      ...
      expect.objectContaining({
        ...
        status: "paused",
        statusVariant: "warn",
        issues: ["symbol_operational_paused"],
      }),
      expect.objectContaining({
        symbol: "SOLUSDT",
        status: "unknown",
