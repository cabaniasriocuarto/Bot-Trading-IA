...
export default function StrategyDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const [allStrategies, allBacktests, allTrades, modernRunDetails] = await Promise.all([
        ...
        loadStrategyRunDetails(strategyId),
      ]);
      ...
      setStrategyRunDetails(modernRunDetails);
      setTrades(allTrades);
  ...
  const latestStrategyRunDetail = useMemo(
    () => strategyRunDetails.find((row) => row.strategy_detail?.series_available) || strategyRunDetails[0] || null,
    [strategyRunDetails],
  );
  const latestStrategyDetail = latestStrategyRunDetail?.strategy_detail || null;
  ...
  const costConfigRows = useMemo(
    () =>
      strategyRunDetails.slice(0, 8).flatMap((row) => {
        const costsModel = row.strategy_detail?.costs_model;
        if (!costsModel) return [];
        return [
          {
            label: row.run_id,
            fees: costsModel.fees_bps,
            slippage: costsModel.slippage_bps,
            funding: costsModel.funding_bps,
          },
        ];
      }),
    [strategyRunDetails],
    ...
    <div className="space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-100">
              ...
              {" "}Las series de equity/costos de abajo ya usan `strategy_detail` del endpoint moderno de runs; el histograma sigue viniendo del endpoint de trades.
            </div>
            {latestEvidenceRun ? (
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <CardDescription>Serie detallada del ultimo run moderno disponible. Se muestra como evidence derivada, no como truth.</CardDescription>
          <CardContent>
            {latestStrategyDetail?.equity_curve?.length ? (
              <EquityDrawdownChart
                data={latestStrategyDetail.equity_curve.map((point) => ({
                  ...point,
                  label: point.time.slice(5, 10),
                }))}
              />
            ) : (
              <p className="text-sm text-slate-400">Sin detalle moderno de equity para graficar.</p>
            )}
          </CardContent>
        </Card>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        ...
        <Card>
          ...
          <CardDescription>Configuracion de costos usada en las corridas recientes desde `strategy_detail`. Es evidence derivada del run, no truth base.</CardDescription>
          <CardContent className="space-y-3">
            ...
            {latestStrategyDetail?.costs_breakdown ? (
              <p className="text-xs text-slate-400">
                Ultima corrida moderna: costo total {fmtNum(latestStrategyDetail.costs_breakdown.total_cost)}
                {" "}· gross {fmtNum(latestStrategyDetail.costs_breakdown.gross_pnl_total)}
                {" "}· net {fmtNum(latestStrategyDetail.costs_breakdown.net_pnl_total || latestStrategyDetail.costs_breakdown.net_pnl || 0)}
              </p>
            ) : null}
          </CardContent>
        </Card>
...
async function loadStrategyRunDetails(strategyId: string): Promise<BacktestCatalogRun[]> {
  try {
    const response = await apiGet<BacktestCatalogRunsResponse>(
      `/api/v1/runs?strategy_id=${encodeURIComponent(strategyId)}&limit=8`,
    );
    const items = Array.isArray(response.items) ? response.items : [];
    const details = await Promise.all(
      items.map(async (row) => {
        try {
          return await apiGet<BacktestCatalogRun>(`/api/v1/runs/${encodeURIComponent(row.run_id)}`);
        } catch {
          return row;
        }
      }),
